...
query = "SET QUERY_GOVERNOR_COST_LIMIT 0 SELECT DISTINCT t.SEQ_BUP_PESSOA --, e.* FROM BUP_TRILHA_PESSOA_DICOM t, prod.SEQ_BIP_PESSOA) AND (prod.SEQ_BIP_PESSOA_PRODUTO = e.SEQ_BIP_PESSOA_PRODUTO) AND (e.NOM_EVENTO LIKE '%LEI%' OR e.NOM_EVENTO LIKE '%DIREITO%' OR e.NOM_EVENTO LIKE '%JURIDIC%' OR e.NOM_EVENTO LIKE '%TRIBUT%')"
...
query = "SET QUERY_GOVERNOR_COST_LIMIT 0 SELECT DISTINCT emp.SEQ_BUP_PESSOA FROM BUP_PESSOA_EMPREGO_DICOM emp (NOLOCK) WHERE (emp.NOM_EMPRESA NOT LIKE '%FUNDACAO GETULIO VARGAS%' AND emp.NOM_EMPRESA NOT LIKE '%VARGAS%' AND emp.NOM_EMPRESA NOT LIKE '%FGV%' AND emp.NOM_EMPRESA NOT LIKE '%AVON%' AND emp.NOM_EMPRESA NOT LIKE '%BRADESCO%') AND (emp.DES_CARGO LIKE '%ADVOGAD%' OR emp.DES_CARGO LIKE '%JUIZ%' OR emp.DES_CARGO LIKE '%DESEMBARGAD%' OR emp.DES_FUNCAO LIKE '%ADVOGAD%' OR emp.DES_FUNCAO LIKE '%JUIZ%' OR emp.DES_FUNCAO LIKE '%DESEMBARGAD%')"
...
query = "SET QUERY_GOVERNOR_COST_LIMIT 0 SELECT DISTINCT T.SEQ_BUP_PESSOA--, NOM_CURSO, STA_SITUACAO FROM BUP_TRILHA_PESSOA_DICOM t INNER JOIN BIP_PESSOA_DICOM bipP ON T.SEQ_STG_PESSOA = BIPP.SEQ_STG_PESSOA INNER JOIN BIP_PESSOA_PRODUTO_DICOM prod ON BIPP.SEQ_BIP_PESSOA = PROD.SEQ_BIP_PESSOA INNER JOIN BIP_PESSOA_PRODUTO_CURSO_DICOM c ON C.SEQ_BIP_PESSOA_PRODUTO = PROD.SEQ_BIP_PESSOA_PRODUTO LEFT OUTER JOIN BUP_PESSOA_EMPREGO_DICOM emp ON EMP.SEQ_BUP_PESSOA = T.SEQ_BUP_PESSOA WHERE(c.FLG_CUMPRIMENTO <> 'S' or c.FLG_CUMPRIMENTO is null) -- N�O PEGAR PESSOAS QUE EST�O CURSANDO ALGUMA DISCIPLINA AND c.COD_CURRICULO NOT  LIKE '%OCW%' -- EX-ALUNOS DE CURSOS QUE N�O S�O GRATUITOS AND ((emp.NOM_EMPRESA NOT LIKE '%FUNDACAO GETULIO VARGAS%' AND emp.NOM_EMPRESA NOT LIKE '%VARGAS%' AND emp.NOM_EMPRESA NOT LIKE '%FGV%' and emp.NOM_EMPRESA NOT LIKE '%AVON%' AND emp.NOM_EMPRESA NOT LIKE '%BRADESCO%') or emp.NOM_EMPRESA is null) AND (c.NOM_CURSO LIKE '%DIREITO%' OR c.NOM_CURSO LIKE '%JURIDC%' OR c.NOM_CURSO LIKE '%LEI%' OR c.NOM_CURSO LIKE '%TRIBUT%')"
